...
export class Game {
    constructor() {
        ...
        this.initialPlacements = new Map(); // Track original positions and types (key: "row,col")
        this.lastGameResult = null; // Track last game result for multiplier
        this.lastBlackValue = 0; // Track previous round's black value to ensure it never decreases
    ...
    handleMoveResult(moveResult, color) {
    ...
    getBlackPiecesForGame(gameNumber) {
        ...
        const availablePieces = ['queen', 'rook', 'bishop', 'knight', 'pawn'];
        const randomPieces = [];
    ...
    balanceBlackPieces(blackPieces) {
        const gameNumber = this.gameState.currentGameNumber;
    ...
    getRandomEmptyPosition(minRow, maxRow) {
        let attempts = 0;
        while (attempts < 100) {
    ...
    setupNextGame() {
        ...
        const survivingPieceCounts = new Map();
        ...
        const originalPieceCounts = new Map();
        const originalPiecePositions = new Map();
